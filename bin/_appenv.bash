...
#
# Usage: ./_appenv.bash <FILE>.appenv
#

# === MAIN ====================================================================
BEFORE=`python3 -c "import os,sys,json;d=(dict((_,os.environ[_]) for _ in sorted(os.environ)));print(json.dumps(d))"`
. $1 > /dev/null
AFTER=`echo $BEFORE | python3 -c "import json,sys,os;b=json.loads(sys.stdin.read());d=dict((_,os.environ[_]) for _ in os.environ if b.get(_)!=os.environ[_]);[print('appenv_set \"{0}\" \"{1}\";'.format(v,k)) for v,k in d.items()]"`
echo $AFTER
# EOF
